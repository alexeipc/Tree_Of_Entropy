...
if __name__ == "__main__":
    ...
    config = GRPOConfig(
        output_dir="./trl_dapo_deepmath_full_set_output",
        num_train_epochs=1,
        max_steps=-1,
        learning_rate=1e-6,
        bf16=True,

        # 2 sequences/GPU x 8 GPUs = 16 completions per microstep.
        per_device_train_batch_size=2,
        gradient_accumulation_steps=1,

        # Four completions per prompt; 16 completions = four prompts.
        num_generations=4,
        ...
        save_total_limit=3,
        report_to=["wandb"],
        run_name="raw-dapo-deepmath-llama-3b-full-set-8gpu",
        remove_unused_columns=False,
        gradient_checkpointing=True,

        # DAPO does not need a reference model for a KL penalty.
        beta=0.0,
        loss_type="dapo",
    ...
    trainer.train(
        # resume_from_checkpoint="./trl_dapo_deepmath_full_set_output/checkpoint-100"
    )
    trainer.save_model("./trl_dapo_deepmath_full_set_final")
